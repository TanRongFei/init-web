    ...
    <div class="m-select-con">
      ...
      <div v-show="isAdvanSearch" class="m-select-bd p15">
        <el-form :rules="advSearchRules" size="small" :model="advSearchForm">
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="管理员(变更前)：" prop="beforUser">
                <el-input
                  v-model="beforUser"
                  style="width: 250px;"
                  placeholder="请输入"
            ...
            <el-col :span="12">
              <el-form-item label="管理员(变更后)：" prop="afterUser">
                <el-input
                  v-model="afterUser"
                  style="width: 250px;"
                  placeholder="请输入"
          ...
          <el-row>
            <el-col>
              <el-form-item label="状态：" prop="custState">
                <el-radio-group v-model="advSearchForm.chgStatus" :max="1">
                  <el-radio label="10">保存</el-radio>
                  <el-radio label="15">提交</el-radio>
                  <el-radio label="20">审批不通过</el-radio>
                  <el-radio label="30">审批通过</el-radio>
                </el-radio-group>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <div class="mt15">
      ...
      <div class="table-list">
        <el-table :data="tableData" highlight-current-row border stripe style="width: 100%;">
          <el-table-column prop="chgStatus" label="状态" width="155%">
            <template slot-scope="stData">
              <el-tag v-if="stData.row.chgStatus===10" effect="plain">保存</el-tag>
              <el-tag v-else-if="stData.row.chgStatus===15" effect="plain">提交</el-tag>
              <el-tag v-else-if="stData.row.chgStatus===20" effect="plain">审核通过</el-tag>
              <el-tag v-else-if="stData.row.chgStatus===30" effect="plain">审核不通过</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="chgCode" label="变更编号 " width="155%" />
          <el-table-column prop="createDate" label="申请时间" width="155%" />
          <el-table-column prop="compName" label="企业名称" width="155%" />
          <el-table-column prop="cuerName" label="管理员(变更前)" width="155%" />
          <el-table-column prop="cuerNameChg" label="管理员(变更后)" width="155%" />
          <el-table-column prop="createdName" label="经办人" width="155%" />
          <el-table-column prop="createdMobile" label="经办人手机号" width="155%" />
...
<script>
import Admin from '@/api/CRM/adminchange'
export default {
  name: 'Administrator',
  data() {
    return {
